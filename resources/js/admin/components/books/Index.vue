  ...
  <div>
    ...
    <div class="container">
      <main class="content" role="main">
        <div>
          <h1>Bücher</h1>
          <router-link :to="{ name: 'book-create' }" class="btn-add">
            <span>Hinzufügen</span>
          </router-link>
          <div class="list-items" v-if="books.length">
            <draggable v-model="books" @end="updateOrder" ghost-class="draggable-ghost" tag="div">
              <div
                :class="[book.publish == 0 ? 'is-disabled' : '', 'list-item', 'is-sortable']"
                v-for="book in books"
                :key="book.id"
              >
                ...
                <div class="list-item-action">
                  <a
                    href="javascript:;"
                    :class="[book.publish == 1 ? 'icon-eye' : 'icon-eye-off', 'icon-mini']"
                    @click.prevent="toggleStatus(book.id,$event)"
                  ></a>
                  <router-link
                  ...
                  <a
                    ...
                    @click.prevent="clone(book.id,$event)"
                  ></a>
                  <a
                    href="javascript:;"
                    class="icon-trash icon-mini"
                    @click.prevent="destroy(book.id,$event)"
                  ></a>
                </div>
              </div>
            </draggable>
